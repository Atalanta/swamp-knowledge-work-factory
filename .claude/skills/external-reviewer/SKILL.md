...
- stage `prompt`s (the reviewer prompt is yours to author — see the
  "Authoring the reviewer prompt" note below),
...
- **Authoring the reviewer prompt.** The `prompt` you pass to
  `external-review-findings` is per-project content — this bundle is deliberately
  silent about *what* the reviewer should look for, the same way the factory is
  silent about *who* reviews. A good prompt instructs the external agent to read
  the work products from swamp (via `swamp data query` in the trusted `cwd`),
  review adversarially, and return ONLY findings JSON matching the factory's
  `kind: findings` schema (severities `critical|high|medium|low`, stable id
  prefixes so passes don't collide, verify prior resolutions on re-review).
  Rather than inline a large prompt in the factory YAML, you can keep prompts as
  reusable, versioned, publishable files by packaging them as a swamp **skill
  extension** and feeding a file in as the `prompt` input — e.g.
  `@atalanta/ts-review-lenses` is one such collection (three adversarial
  TypeScript review lenses). This is an illustrative pattern, not a dependency:
  any prompt source works, and the bridge neither requires nor knows about it.
